...
assert.match(app, /const dnaHelixStrands = \[/, 'Hero should define DNA helix strands for the animated visual');
assert.match(app, /function buildDnaHelixPath/, 'Hero should build DNA helix geometry from pointer position');
assert.match(app, /const dnaRungs = /, 'Hero should render detailed DNA connector rungs');
assert.match(app, /distortedDnaHelixStrands/, 'Hero should render pointer-distorted DNA helix strands');
assert.match(app, /ntn-dna-wave-field/, 'Hero should render the DNA wave field around the image');
assert.match(app, /strokeDashoffset/, 'DNA helix strands should animate flowing energy along the strokes');
assert.match(app, /useSpring/, 'Hero magnetic wave hover should use spring motion for smooth cursor following');
assert.match(app, /handleDnaPointerMove/, 'Hero should update DNA waves from pointer movement');
assert.match(app, /onPointerMove=\{handleDnaPointerMove\}/, 'Hero visual should react to pointer movement');
assert.match(app, /onPointerLeave=\{handleDnaPointerLeave\}/, 'Hero visual should reset DNA waves when hover ends');
...
assert.doesNotMatch(app, /ntn-wave-field absolute inset-x-\[-18%\]/, 'Hero should replace the previous wave field animation');
assert.doesNotMatch(app, /ntn-data-stream/, 'Hero should remove the treadmill-like data stream layer');
assert.doesNotMatch(app, /perspective\(760px\) rotateX\(64deg\)/, 'Hero should not render the tilted conveyor-belt effect');
